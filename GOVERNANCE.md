...
- [ ] Review `data/audit/*-discover.md` since the last quarterly — **any symbol appearing in ≥3 consecutive reports must receive a written verdict** (add / reject-with-reason / defer-with-date). An unresolved third appearance is itself a review trigger. (Gap #19)
- [ ] Before reading any ranking or shortlist, internal or external: **ask how the candidate set was generated.** If the candidate set is the coverage pool, the information ceiling of the ranking is "which of the things I already cover do I prefer". (Gap #18)
...
18. **The discovery script's candidate set is generated from the universe, so the universe can only grow into its own neighbourhood** (v2.9.19)

    `scripts/discover-candidates.js` is the only intake path that runs on a schedule. Its method, from the file header and confirmed in `main()`: for each **active equity ticker already in the universe**, call Yahoo `recommendationsBySymbol`; aggregate peer mentions; keep symbols that are **not in the universe**, have **market cap >= $1B**, and were **mentioned by >=2 existing tickers**.

    **The reachable set is therefore the one-hop Yahoo peer neighbourhood of the current 48 active equity names, intersected with a >=2-mention rule.** A company can only be seen if at least two names already held list it as a peer. And Yahoo's peers are a **similarity** relation - same sector, industry and size band - not a **supply-chain adjacency** relation. So the expansion direction the engine can travel is *more of the same kind of company*, never *the next link in the chain*. The repo's own layer model is an adjacency model; its intake is a similarity model. They are not the same graph.

    **Empirical signature - six discover reports, from this repo's own `data/audit/`:**

    | report | 2026-05-08 | 2026-05-19 | 2026-06-01 | 2026-07-01 | 2026-08-01 | 2026-09-01 |
    |---|---|---|---|---|---|---|
    | candidates | 15 | 13 | 11 | 10 | **8** | 9 |

    Monotone decline. Across all six reports only **18 distinct symbols** ever appeared - `AAOI AAPL ACLS ADI ALAB CRDO CSCO INTC KLIC LSCC NBIS NFLX NOW PLAB SOUN TER TXN ZS` - of which several are already in the universe and several are plainly off-thesis (`AAPL NFLX NOW ZS CSCO SOUN`). **This is not "there are fewer opportunities." It is the signature of an exhausted search radius: the peer neighbourhood is being absorbed into the universe, and the method has a fixed point.**

    **Control evidence.** A separate 2026-09-09 session working the ABF-substrate and HBM4E lines produced eight names with checkable chain positions - `MKSI ONTO CAMT BESIY MTRN NVT FLEX AEIS`. **None of the eight appears in any of the six discover reports.** `MKSI` is the sharpest case: it is an obvious sector-and-size peer of `AMAT`, `LRCX`, `KLAC`, `ENTG` and `TER`, all five of which are active in the universe, and the similarity path still did not surface it. Whatever the peer relation selects on, it is not the thing this engine needs.

    **Not the same as Gap #15, and they should not be merged.** #15 is about **direction** - the universe is built up the supply chain while the frame says capture moves down it. #18 is about **closure** - the candidate set is a function of the current holdings. A universe could be built in the correct direction and still be closed; it could be open and still point the wrong way.

    **The general rule this instantiates.** Before reading any ranking, shortlist or "which one should I pick" output, **ask how the candidate set was generated.** When the candidate set is the coverage pool, the information ceiling of the ranking is *"which of the things I already cover do I prefer"* - not *"which is best in this space"*. The failure family is availability skew: the distribution of evidence that can be retrieved is not the true distribution, and the direction of the skew is set by whoever produces the evidence. Here the producer is a recommendation algorithm seeded with the portfolio.

    **What is not the fix.** Loosening `MIN_PEER_MENTIONS` from 2 to 1, or `MIN_MARKETCAP_B` from 1 to 0.5, draws the same circle larger. The circle's centre is the problem.

    **Proposed second intake path (not implemented).** Seed from `layers.json`'s physical step definitions rather than from holdings: ask *"who in the world occupies L5_5"* instead of *"who are TTMI's peers"*. **Honest cost:** this cannot be fully automated - no free API enumerates a layer's global supplier set - so the realistic form is a **per-layer sweep at the quarterly review**, one layer at a time, not a monthly cron. That makes it slower than the existing path, and it should be additive rather than a replacement: the peer path is cheap and does surface real candidates (see #19).

    **Deliberately not fixed here.** No script changed, no scoring field touched. Two tickers were added at `watchlist` in the same commit (`MKSI`, `KLIC`) - additions are permitted under Gap #13 and are excluded from gate A, and the frozen roster in `data/ab-track/frozen-2026-08-17.json` is untouched. **Adding two names by hand is a patch, not a fix**; it is recorded as such so the next quarterly review does not mistake it for one.

19. **Discovery surfaced the same candidate three consecutive months and no rule required a verdict** (v2.9.19)

    `KLIC` appears as a scored candidate in `2026-07-01-discover.md`, `2026-08-01-discover.md` and `2026-09-01-discover.md`. The 2026-09-01 entry is complete: market cap $4B, **peer mentions 2 (sources: ENTG, AMKR)**, an industry-verification step that **corrected** the suggested layer, a drafted thesis and drafted falsification signals. **It was not triaged on any of the three occasions.**

    The script is surfacing-only by design and that design is correct - `NEVER writes to universe.json` is the right boundary, and Tier 1's automation limit exists for good reasons. **What is missing is the other half: nothing anywhere obliges a human to dispose of what was surfaced.** The audit trail therefore records that the machine worked, and conceals that nothing happened.

    **The general form:** a surfacing-only system with no forced-disposition rule degrades silently into a no-op, and it degrades in the direction that looks healthy - reports keep being written, so the process appears live.

    **Rule added to the Tier 2 checklist:** any symbol appearing in **>=3 consecutive** discover reports must receive a written verdict - add, reject-with-reason, or defer-with-date - and an unresolved third appearance is itself a review trigger. **Partially discharged in this commit:** `KLIC` added at `watchlist` with the reason recorded on the entry.

> **Provenance (v2.9.19, 2026-09-09):** **surfaced by the repo owner, not by the engine and not by the assistant.** The owner asked whether the project has a bug because we only ever evaluate the same few dozen tickers, so how would anything new be found. He was applying a criterion the assistant had landed in its own notes roughly thirty minutes earlier - *a ranking's candidate set is the coverage pool; the information ceiling is "which of the things I cover do I prefer"* - written up against a sell-side sector note and **never turned on this repository.** The assistant's own standing remedy for that class of miss is *after landing a criterion, re-scan your own current output with it*; it did not run, and an outside reader executed it instead. **That is the more durable finding: a criterion that is written down but not re-applied to the writer's own live systems has not been adopted, only recorded.** Investigation was code-first - the mechanism was read out of `discover-candidates.js`, the decline and the 18-symbol census were counted from `data/audit/`, and the `KLIC` triage failure was confirmed by reading the report text rather than inferred from counts. Per `inspired_loop` and `techpull_gate`: **no script changed, no scoring field created, no existing ticker's PASS/FAIL state moved, no portfolio motion.** Two `watchlist` additions are documented on their own entries and are explicitly labelled a patch.

20. **A ticker's `layer` is a foreign key into two files and nothing validated it; both failure modes are silent** (v2.9.20)

    Surfaced while correcting the repository description, which claimed a 19-layer model. `layers.json` defines **27**. The universe uses **28 distinct layer ids**, and `sector-benchmarks.json` carries **22**. Three sets that were assumed identical, never compared.

    | | count | detail |
    |---|---|---|
    | Layers defined in `layers.json` | 27 | one of them, `L8`, carries no ticker |
    | Distinct layer ids used by equities | 28 | **`L8_OPT_MAT` (AXTI) and `L9_MINER_CONVERT` (APLD, CIFR, IREN, WULF, HUT) are not defined anywhere** |
    | Layers with an entry in `sector-benchmarks.json` | 22 | 6 used layers have none |
    | Equities scored against the fallback constant | **12 of 84 (14.3%)** | `L11_FUEL` CCJ · `L5_5` TTMI · `L8_OPT_MAT` AXTI · `L_DCOMP` CRCL · `L_EMBI` ISRG CGNX PH TSLA VPG · `L_SPACE` RKLB ASTS MOG |

    **Why both modes are silent, which is the point.**

    - *Undefined in `layers.json`*: `audit-layers.js` builds its metrics table by iterating `layersData.layers`. A ticker in an undefined layer is **not reported as broken — it is not reported at all.** Six tickers were invisible to the monthly layer audit.
    - *Missing benchmark*: `update-valuations.js` reads `benchmarks[ticker.layer] || benchmarks.default` and falls through to a hardcoded `medianForwardPE: 22, medianEVRevenue: 5`. `assessPricingApplicability` checks three things — crypto, whether a positive forward P/E or EV/Revenue exists, and component coverage — and **none of them is which benchmark was used.** The score comes back `pricingApplicable: true`. Forward P/E carries 30% of the composite and EV/Revenue 25%, so for these twelve names **55% of the pricing score is deviation from a constant rather than from peers.**

    **Measured blast radius on the funnel: one name, and it is robust.** `TTMI` is the only affected ticker that passes. Solving `score = 3 − 2.5·(median − value)/median` against its components (forward P/E 19.06, EV/Rev 4.09, analyst upside 63.1% → 1.0, momentum +22.6% relative → 3.8) shows the layer median would have to fall below **~13 P/E** before it exits the hysteresis band at 3.2. `L5_5` is the PCB and ABF-substrate layer during a substrate shortage; its true median is above 22, not below 13. **Correcting the benchmark would make TTMI look cheaper, not more expensive.**

    ⚠️ **The damage is entirely on the other side, and it is the side nobody looks at.** The remaining ten are scored against P/E 22 and marked expensive: `TSLA` at forward P/E **169.7**, `RKLB` at **1486**, `ASTS` **negative** (forced to 5 by the net-loss rule), `CRCL` at **61**, `CCJ` at **39.5**. Whether any of those is actually expensive depends on what comparable companies in an embodied-AI, launch-services, stablecoin or nuclear-fuel layer trade at — **and that number does not exist.** A false positive gets scrutinised because it entered the funnel. **A false negative appears in no report at all**, so nothing prompts anyone to check it. This is family 1 of the failure table in its hardest-to-catch form: an exclusion leaves no artefact.

    **Relationship to Gap #18.** Two independent mechanisms narrowing the same funnel, both silent. #18 stops new names being *found*; #20 marks existing names in unbenchmarked layers *expensive*. Neither produces an error, a warning, or a row in any report.

    **What was done here.** `audit-layers.js` now performs three deterministic set comparisons and reports them at the top of the monthly report, and the workflow opens an issue on `schema_violations != 0`. **Deliberately not a non-zero exit:** the "Commit audit reports" step runs after the audit step, so failing there would discard the report that records the finding. Forced disposition comes from the issue instead — per Gap #19, surfacing without a disposition rule decays into a no-op.
